<?php
    ...
    class moduloArticulo{
        public static $conn;
        public static function init () {
            self::$conn = connect();
        }

        function getArticulo( $id, $area, $role ){
            $sql = "SELECT 
                u.lastName,
                u.firstName,
                a.idArticulo,
                ...
                if( $role === 'researcher' ){
                    $sql .= " ON u.userId = a.userId WHERE u.userId = :userId ORDER BY fechaArticulo DESC ";
                }else if( $role === 'leadership' && $area === 'ISC' ){
                    $sql .= " ON u.userId = a.userId WHERE u.area = 'ISC' ORDER BY fechaArticulo DESC ";
                }else if( $role === 'admin' ){
                    $sql .= " ON u.userId = a.userId ORDER BY fechaArticulo DESC ";
                }
